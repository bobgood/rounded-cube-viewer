// marching_cubes.js — classic Marching Cubes (Lorensen & Cline) isosurface
// extraction from a scalar field on a regular grid. Tables are Paul Bourke's
// reference edge/tri tables (http://paulbourke.net/geometry/polygonise/).
//
// extractIsosurface(values, n, iso, origin, step) → Float32Array of triangle
// vertices (scene units). `values` is length n³, C-order index (i*n + j)*n + k;
// grid point (i,j,k) sits at origin + (i,j,k)*step on each axis.
...
export function extractIsosurface(values, n, iso, origin, step) {
  const out = [];
  const idx = (i, j, k) => (i * n + j) * n + k;
  const ox = origin, oy = origin, oz = origin;
  const val = new Float64Array(8);
  const px = new Float64Array(8), py = new Float64Array(8), pz = new Float64Array(8);
  const ex = new Float64Array(12), ey = new Float64Array(12), ez = new Float64Array(12);
  ...
  for (let i = 0; i < n - 1; i++) {
    for (let j = 0; j < n - 1; j++) {
      for (let k = 0; k < n - 1; k++) {
        let cubeIndex = 0;
        for (let c = 0; c < 8; c++) {
          const ci = i + CORNER[c][0], cj = j + CORNER[c][1], ck = k + CORNER[c][2];
          const v = values[idx(ci, cj, ck)];
          val[c] = v;
          px[c] = ox + ci * step;
          py[c] = oy + cj * step;
          pz[c] = oz + ck * step;
        ...
        for (let e = 0; e < 12; e++) {
          if (edges & (1 << e)) {
            const a = EDGE_CONN[e][0], b = EDGE_CONN[e][1];
